...
console.log('%c compressed ', 'background: #222; color: #bada55');
		...
		p.paused = function(value) {
			...
			if (value != this._paused) if (tl) {
				...
				if (!value && elapsed !== 0 && this._initted && this.duration()) {
					...
					this.render(raw, (raw === this._totalTime), true); //in case the target's properties changed via some other tween or manual updateElements by the user, we should force a render.
				}
			}
			if (this._gc && !value) {
				this._enabled(true, false);
 ...
 */
		...
		_tickerActive = false; //ensures that the first official animation forces a ticker.tick() to updateElements the time when it is instantiated
			...
			Draggable = function(target, vars) {
				...
				this.applyBounds = function(newBounds) {
					...
					if (newBounds && vars.bounds !== newBounds) {
						...
						return self.updateElements(true);
					}
					syncXY(true);
					calculateBounds();
					if (hasBounds) {
				...
				this.updateElements = function(applyBounds, ignoreExternalChanges) {
					var x = self.x,
						y = self.y;
					updateMatrix();
					if (applyBounds) {
